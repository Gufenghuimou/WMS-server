...
(() => {
                    ...
                    <div class="right-part" style="flex: 1; min-width: 0;">
                        <div class="card-header">
                            <div class="card-info">
                                ...
                                <div class="text-truncate info-name" style="font-size: 0.9rem; font-weight: 500;" title="${ group.name }">${ group.name || t('asset_view.dest_lbl') }</div>
                            </div>
                        </div>
    ...
    function bindEvents() {
        const targetForms = ['assetToggleForm', 'stopForm', 'assetItemEditForm'];
    ...
    async function handleFormSubmit(e) {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                if (form.classList.contains('require-form') || form.action.includes('/api/request_asset_by_pn')) {
                    let card = form.closest('.asset-card');
                    if (card) window.cancelEdit({stopPropagation: () => {}}, card.id);
                    return;
                }

                if (form.classList.contains('admin-edit-form')) {
                    let card = form.closest('.asset-card');
                    if (card) {
                        window.cancelEdit({stopPropagation: () => {}}, card.id);
    ...
    window.updateMiniChartUI = function (groupId) {
        ...
        container.innerHTML = `
            ...
            <div style="width: 100%; height: 6px; background: #ecf0f1; border-radius: 4px; display: flex; overflow: hidden; box-shadow: inset 0 1px 2px rgba(0,0,0,0.05);">        
                ...
                ${broken_qty > 0 ? `<div style="width: ${broken_pct}%; background: #95a5a6; transition: 0.3s;" title="${t('asset_view.stopped')}: ${ broken_qty }"></div>` : ''}
                ${used_qty > 0 ? `<div style="width: ${used_pct}%; background: rgba(231, 76, 60, 0.5); transition: 0.3s;" title="${t('asset_view.using')}: ${ used_qty }"></div>` : ''}
            </div>
        `
    }
