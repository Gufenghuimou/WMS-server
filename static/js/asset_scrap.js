(() => {
    ...
    let scanInput, scrapList, countDisplay;

    // 拉取后端数据
    window.initAssetScrapPage = async function () {
    ...
    window.addAssetToTable = function(item, isFromFetch = false) {
        ...
        const deleteTitle = isFromFetch ? 'Must unstop asset firstly' : t('asset_scrap.title_remove');
        ...
        if (currentEmptyState && currentEmptyState.style.display !== 'none') currentEmptyState.style.display = 'none';
    ...
    window.updateUI = function() {
        ...
        const currentActionButton = document.querySelector('button[onclick="openScrapModal()"]');

        if (count > 0) {
            if (currentEmptyState) currentEmptyState.style.display = 'none';
            if (currentActionButton) currentActionButton.disabled = false;
        } else {
            if (currentEmptyState) currentEmptyState.style.display = 'table-row';
            if (currentActionButton) currentActionButton.disabled = true;
        }
    };
})();
