...
(() => {
    ...
    window.initAdminPage = async function() {
        ...
        if (editorCanvas) eCtx = editorCanvas.getContext('2d');
    ...
    function bindCanvasEvents() {
        ...
        editorCanvas.onmouseleave = () => { isDragging = isSelecting = false; drawEditor(); }
    }
    ...
    function updatePropPanel() {
        ...
        if (selectedRacks.length === 1) {
            ...
            pName.value = t('admin.selected_count').replace('{count}', selectedRacks.length);
            pW.value = ''; pH.value = '';
    ...
    window.addNewRack = function() {
        editorRacks.push({ name: t('admin.new_rack_name'), color: "#e0f2fe", x: 30, y: 30, w: 60, h: 120 });
