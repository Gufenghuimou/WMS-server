...
(() => {
    ...
    window.initMapEngine = async function() {
        ...
        mapEl = document.getElementById('footerWarehouseCanvas');
        if (mapEl) {
    ...
    window.openFooterMap = function(targetLoc) {
        ...
        if (targetLoc) {
            subtitle.innerHTML = `<span style="color: #ff3e3e; font-weight: bold;">${t('base.target_loc')}${targetLoc}</span>${t('base.drag_tip')}`;
        }
    ...
    window.toggleMapExpansion = function() {
        ...
        if (isExpanded) {
            ...
            // 收起时，把文字恢复成默认介绍
            subtitle.innerHTML = t('base.map_legend');
        }
...
(() => {
    ...
    function renderMessages(messages) {
        ...
        messages.forEach(msg => {
            ...
            const roleTag = msg.role === 'admin' ? t('base.admin_tag') : '';
...
(() => {
    ...
    // superadmin 专属心跳检测
    window.initSecuritySystem = function() {
...
// 其他零散工具模块
// ==========================================
window.initBaseUIComponents = function() {
    ...
    if (globalSearch && clearBtn) {
        // 初始化状态
        clearBtn.style.display = globalSearch.value.length > 0 ? '' : 'none';
        // 监听输入
    ...
    const updateCount = () => {
        ...
        const simcardStockinCountDisplay = document.getElementById('simcardStockinCountDisplay');
        // 消耗品
        ...
        // Simcard
        if (simcardStockinCountDisplay) {
            let count = 0;
            const container = document.getElementById('assetStockInForm') || document;
            container.querySelectorAll('input[name="icc_id"]').forEach(input => {
                if (input.value.trim() !== '') count++;
            });
            simcardStockinCountDisplay.innerText = count;
        }
    };
