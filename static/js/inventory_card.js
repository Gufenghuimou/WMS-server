(() => {
    ...
    function bindEvents() {
        ...
        grid.addEventListener('wheel', (e) => {
            if (e.deltaY !== 0 || e.deltaX !== 0) {
                ...
                let direction = Math.abs(e.deltaX) > Math.abs(e.deltaY) ? Math.sign(e.deltaX) : Math.sign(e.deltaY);
        ...
        grid.addEventListener('mousemove', window.stopAutoPlay, { once: true });
        ...
        if (realStockInput) realStockInput.oninput = hideOutError;
        if (outQtyInput) outQtyInput.oninput = hideOutError;

        // 🌟 统一拦截表单提交 (代替全局拦截)
        const outForm = document.getElementById('outForm');
        const editForm = document.getElementById('editForm');
        if (outForm) outForm.onsubmit = handleFormSubmit;
    ...
    function scrollWidgetsControl() {
        ...
        if (leftBtn && rightBtn) {
            ...
            leftBtn.onclick = () => stepCoverFlow(-5);
            rightBtn.onclick = () => stepCoverFlow(5);
        }
    }
    ...
    async function handleFormSubmit(e) {
        ...
        if (submitBtn) {
            ...
            submitBtn.innerHTML = `<i class="material-icons" style="animation: spin 1s linear infinite;">autorenew</i> ${t('card.processing')}`;
        }
    ...
    window.startAutoPlay = function() {
        ...
        document.querySelectorAll('.custom-modal').forEach(modal => {
            if (window.getComputedStyle(modal).display !== 'none') isModal = true;
        });
        if (document.documentElement.matches(':hover') || isModal) return;
        
        if (autoPlayTimer) clearInterval(autoPlayTimer);
        autoPlayTimer = setInterval(() => {
            // 🌟 核心 SPA 修复：自杀机制，离开页面立刻销毁定时器
    ...
    window.updateCoverFlow = function() {
        ...
        cards.forEach((card, i) => {
            ...
            if (diff === 0) {
                ...
                card.classList.add('is-left'); card.classList.remove('is-flipped');
                ...
                card.classList.add('is-right'); card.classList.remove('is-flipped');
                let tx = 375 + ((diff - 1) * 200);
                card.style.setProperty('--tx', `${tx}px`);
                card.style.setProperty('--tz', `${-60 - (diff * 5)}px`);
                card.style.setProperty('--z', 50 - diff);
    ...
    window.validateOutForm = function(event) {
        ...
        if (realStock < reqQty) {
            ...
            errorText.innerText = `${t('card.stock_insufficient').replace('{realStock}', realStock).replace('{reqQty}', reqQty)}`;
    ...
    window.openRequestModal = function(id, pn1, name, stock) {
        ...
        qtyInput.value = ''; qtyInput.max = stock;
    ...
    window.submitRequest = async function(event) {
        ...
        btn.disabled = true;
        btn.innerHTML = `<i class="material-icons" style="animation: spin 1s linear infinite;">autorenew</i> ${t('card.submitting')}`;
        ...
        try {
            ...
            if (data.status === 'success') {
                btn.innerHTML = `<i class="material-icons">check</i> ${t('card.req_sent')}`;
    ...
    window.toggleBookmark = async function(event, itemId) {
        ...
        try {
            ...
            if (document.getElementById('favToggleBtn') && document.getElementById('favToggleBtn').classList.contains('active')) {
                window.applyFilters();
            }
        } catch (err) {
