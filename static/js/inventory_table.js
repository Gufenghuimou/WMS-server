...
(() => {
    ...
    window.initInventoryTablePage = async function () {
    ...
    function renderInvTab(data) {
        ...
        const rowsHtml = data.map(item => {
            ...
            if (is_alarming) {
                alarmBadge = `<span class="alarm-badge alarm-yes"><i class="material-icons" style="font-size: 0.8rem; vertical-align: middle;">warning</i> ${t('table.alarm_yes')}</span>`;
            } else {
                alarmBadge = `<span class="alarm-badge alarm-no"><i class="material-icons" style="font-size: 0.8rem; vertical-align: middle;">check</i> ${t('table.alarm_no')}</span>`;
            }
            let hasImage = '';
            if (item.has_image) {
                                        ...
                                        <div class="btn-container">
                                            <div class="detail-edit-btn">
                                                <div class="action-btn">
                                                    <button class="btn-primary scrap-btn" style="background-color: var(--danger-red);">${t('table.scrap_btn')}</button>
                                                    <button class="btn-primary save-btn" style="background-color: var(--primary-blue);">${t('table.save_btn')}</button>
                                                    <button class="btn-primary cancel-btn" style="background-color: #f1f3f4; color: var(--text-main);">${t('table.cancel_btn')}</button>
                                                </div>
                                                <button class="btn-primary edit-btn" style="background-color: var(--primary-green);">${t('table.edit_btn')}</button>
                                            </div>
                                        </div>
                                    </div>
                                </template>
    ...
    function bindTableEvents() {
        ...
        tbody.onclick = (e) => {
            ...
            const targetDetailRow = document.getElementById(`detail-${itemId}`);
            ...
            if (!detailContainer.querySelector('.detail-inner')) {
                const template = targetDetailRow.querySelector('.detail-template');
                if (template) {
            ...
            if (result.status === 'success') {
                ...
                if (badgeTr) {
                    ...
                    if (isAlarming) {
                        ...
                        td.innerHTML = `<span class="alarm-badge alarm-yes"><i class="material-icons" style="font-size: 0.8rem; vertical-align: middle;">warning</i> ${t('table.alarm_yes')}</span>`;
                    } else {
                        td.setAttribute('data-sort', '0');
                        td.innerHTML = `<span class="alarm-badge alarm-no"><i class="material-icons" style="font-size: 0.8rem; vertical-align: middle;">check</i> ${t('table.alarm_no')}</span>`;
                    }
                }
