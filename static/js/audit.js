...
function renderAudit(data) {
    ...
    const blockHtml = Object.entries(data).map(([loc, items]) => {
        ...
        let rowsHtml = items.map(item => {
            ...
            let status = item.status || AUDIT_I18N.status_uncounted;
        ...
        return `
            <div class="location-block ${collapsedClass}" data-loc="${loc.toLowerCase()}">
                <div class="location-header" onclick="this.parentElement.classList.toggle('collapsed')">
                    ...
                    <span class="status-badge ${allDoneClass}">${(AUDIT_I18N.total_items).replace('{count}', items.length)}</span>
                </div>
                ...
                <table class="audit-table">
                    <thead style="position: sticky; top: 0; background: #f1f5f9; z-index: 10; box-shadow: inset 0 -2px 0 var(--border-color);">
                        <tr>
                            <th style="width: 140px;">${AUDIT_I18N.th_pn1}</th>
                            <th style="width: 140px;">PN2</th> <th style="min-width: 250px;">>${AUDIT_I18N.th_name}</th>
                            <th style="width: 80px; text-align: center;">>${AUDIT_I18N.th_book_qty}}</th>
                            <th style="width: 100px; ">${AUDIT_I18N.th_actual_qty}</th>
                            <th style="width: 100px;">${AUDIT_I18N.th_actual_loc}</th>
                            <th style="width: 150px;">${AUDIT_I18N.th_remarks}</th>
                            <th style="width: 80px; text-align: center;">${AUDIT_I18N.th_status}</th>
                            <th style="width: 80px; text-align: center;">${AUDIT_I18N.th_action}</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${rowsHtml}
