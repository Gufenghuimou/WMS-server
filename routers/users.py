...
async def login_page(request: Request):
    ...
    return FileResponse(os.path.join(base_dir, "templates", "login.html"), headers={"Cache-Control": "no-cache"})

@router.post("/login")
async def process_login(request: Request, username: str = Form(...), password: str = Form(...), language: str = Form("")):
    lang = language if language in ("zh", "en", "ja", "vi") else request.state.lang
        ...
        if user:
            if user.role == 'superadmin':
                ...
                if user.last_login_ip and user.last_login_ip != client_ip and is_active:
                    ...
                    return JSONResponse(status_code=403, content={"status": "error", "message": error_msg, "is_security_alert": True}, headers={"Cache-Control": "no-store"})
            ...
            return JSONResponse(content={"status": "success", "redirect_url": "/"}, headers={"Cache-Control": "no-store"})
        else:
            error_msg = t_lang("login.error", lang)
            return JSONResponse(status_code=401, content={"status": "error", "message": error_msg}, headers={"Cache-Control": "no-store"})
